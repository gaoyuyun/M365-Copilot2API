...
func TestAnthropicUsageDefaultsMissingCacheCountersToZero(t *testing.T) {
	src := map[string]any{
		"usage":   map[string]any{"prompt_tokens": float64(10), "completion_tokens": json.Number("2")},
		"choices": []any{map[string]any{"message": map[string]any{"content": "ok"}}},
	}
	rr := httptest.NewRecorder()
	writeAnthropicResult(rr, "m", false, src)
	...
	if usage["input_tokens"] != float64(10) || usage["output_tokens"] != float64(2) ||
		usage["cache_creation_input_tokens"] != float64(0) || usage["cache_read_input_tokens"] != float64(0) {
		t.Fatalf("usage=%#v", usage)
	}
}
...
func TestAnthropicStreamUsageIncludesCacheCounters(t *testing.T) {
	...
	body := rr.Body.String()
	if !strings.Contains(body, `"cache_creation_input_tokens":3`) || !strings.Contains(body, `"cache_read_input_tokens":4`) ||
		!strings.Contains(body, `"output_tokens":0`) || !strings.Contains(body, `"output_tokens":2`) {
